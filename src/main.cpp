...
// Copyright (c) 2013 Troptions developers
...
const string strMessageMagic = "Troptions Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight < 1000 ) {nSubsidy = 1 * COIN;} //allow for block targets to be met
...
static const int64 nMaxAdjustDown = 5; // 5% adjustment down per interval
static const int64 nMaxAdjustUp = 5; // 5% adjustment up per interval
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TroptionsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TroptionsMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running TroptionsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TroptionsMiner terminated\n");
        throw;
    }
}
